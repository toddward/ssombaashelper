...
function doRenew (req, res, next) {
  var token = req.headers['authorization'];
  var headers = { authorization: token };

  if (token) {
    log.d('Renewal of token taking place.  Token: %s.', token);

    fh.service({
      'guid': process.env.SSOGUID,
      'path': '/api/v1/authorization/renew',
      'headers': headers,
      'method': 'POST'
    }, (err, body, response) => {
      if (err) {
      ...
      } else if (response.statusCode === 200 && response.serviceResponse.new_token) {
        return reponse.
      } else {
